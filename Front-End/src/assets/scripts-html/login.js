...
async function verificarUsuario() {
    ...
    try {
        // Chamada para a API
        const response = await fetch('http://localhost:3000/auth/verificacao', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            const userData = {
                ...
                senha: responseData.senha, // Considere não armazenar a senha
                fases: responseData.fases // Incluindo fases
            };
